...
export function validateReportData(data: unknown): ValidationResult {
  const errors: string[] = [];
  ...
  if (dimensions) {
    ...
    dimensions.forEach((dimension, index) => {
      ...
      if (!isNumberInRange(dimensionScore, 0, 100)) {
        ...
        typeof dimensionMaxScore === "number" &&
  ...
  if (!isRecord(data.productShape)) {
    ...
    const firstVersionLook = requireArray(
      data.productShape,
      "firstVersionLook",
      errors,
  ...
  const validationPlan = requireArray(
    data,
    "validationPlan",
    errors,
    "validationPlan"
...
export function normalizeReportData(report: MockReport): MockReport {
  const normalizedScore = report.dimensions.reduce(
