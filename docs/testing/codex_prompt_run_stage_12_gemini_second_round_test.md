# Codex 提示詞：執行 Gemini PoC 第二輪 9 次測試

目前專案已完成第十二階段：

- Gemini 保留第一候選
- 正式 `/evaluate` 仍暫緩切換
- 已補 Gemini 429 retry / backoff
- 已補前端 cooldown
- 已補 `/gemini-poc` 人工內容複查欄位
- 已補可複製 Markdown 測試紀錄
- `docs/gemini-poc-test-results.md` 已新增第二輪測試表
- `docs/gemini-poc-test-plan.md` 已新增第二輪測試說明

現在請幫我執行第二輪 Gemini PoC 測試。
...
1. 驗證 429 retry / backoff 是否有效
2. 驗證前端 cooldown 是否正常
3. 驗證 Gemini 是否仍能穩定輸出合法 JSON
4. 驗證 `validateReportData` 是否仍可通過
5. 驗證 `normalizeReportData` 是否仍正常
6. 執行人工內容複查
7. 更新 `docs/gemini-poc-test-results.md`
8. 回報是否可讓 Gemini 進入正式 provider 候選
...
6. 是否可預覽報告
7. 是否出現 HTTP 429
8. 若出現 429，retry 後是否成功
9. 若 retry 後仍失敗，前端錯誤提示是否清楚
10. cooldown 是否正常運作
...
測試完成後，請在 `docs/gemini-poc-test-results.md` 的「第二輪決策」填入其中一項：
...
3. HTTP 429 不超過 1 次，且 retry 或提示處理正常
4. 明顯內容品質問題不超過 2 次
5. 多數報告 ProductShape 是「是」或「普通」
6. 多數報告 DontBuild 是「是」或「普通」
7. 多數 7 天計畫偏驗證
8. 多數報告語氣冷靜保守
9. 不出現明顯過度承諾
...
5. HTTP 429 出現幾次
6. retry 後成功幾次
7. validate errors 幾次
8. warnings 幾次
9. normalize 修正幾次
10. 明顯內容品質問題幾次
11. 第二輪決策是什麼
12. `docs/gemini-poc-test-results.md` 是否已更新
13. 是否有任何需要我人工再判斷的內容
